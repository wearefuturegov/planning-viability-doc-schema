...
This JSON schema (draft-07) has been developed with the needs of the London Boroughs of Southwark and Tower Hamlets in mind, but should be useful to other London Boroughs, and other UK planning authorities that store, share, or analyse viability assessment data.

It reflects the naming conventions used in the PlanX project, specifically the [PlanX Passport Schema](https://docs.google.com/spreadsheets/d/1_ODPs0x0HAaLx1nrD58x_9gt4SyE11edHRZEq0nO99w/edit?ts=5d9c5f5b#gid=0)

It is work in progress and will evolve.

The following documentation is [autogenerated](https://github.com/adobe/jsonschema2md). HOWEVER, due to a [limitation](https://github.com/adobe/jsonschema2md/issues/73#issuecomment-391615414) in the autogenerator, the following documentation describes the schema that sits within an outer array. The schema file is correct, it's just the documentation below that is slightly different. 

# Scripts
To convert a CSV into JSON that conforms with the schema, the `convertCSVtoJSON.py` script performs that conversion. This is useful for testing purposes. 
...
| ------------------- | ---------- | ------------ | ------------ | ----------------- | --------------------- | -------------------------------------------------------------- |
| Can be instantiated | No         | Experimental | No           | Forbidden         | Forbidden             | [planningViabilitySchema.json](planningViabilitySchema.json) |
